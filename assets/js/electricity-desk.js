...
if (desk) {
  ...
  function render(data) {
    const sourceTime = Date.parse(data.sourceUpdatedAt ?? data.generatedAt);
    const ageMinutes = Math.max(0, (Date.now() - sourceTime) / 60_000);
    const status = statusFor(ageMinutes);
